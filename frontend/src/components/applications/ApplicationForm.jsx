...
const FIELD_CLASS = 'w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/30 bg-white/5 border border-white/10 focus:outline-none focus:border-brand-500 focus:bg-white/8 transition-all duration-200'
const LABEL_CLASS = 'block text-xs font-medium text-white/50 mb-1.5 uppercase tracking-wider'

export default function ApplicationForm({ onSubmit, initialData, submitting }) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm({
    defaultValues: initialData || { company: '', role: '', status: 'Applied', applied_date: '', link: '', notes: '' },
  })
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" noValidate>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <select id="form-status" {...register('status')} className={`${FIELD_CLASS} cursor-pointer`} style={{ colorScheme: 'dark' }}>
            {ALL_STATUSES.map((s) => {
        ...
        <div>
          ...
          <input id="form-date" type="date" {...register('applied_date')} className={FIELD_CLASS} style={{ colorScheme: 'dark' }} />
        </div>
      </div>

      <div>
        <label className={LABEL_CLASS}>Job Link</label>
        <input id="form-link" {...register('link')} placeholder="https://careers.example.com/job/123" className={FIELD_CLASS} />
      </div>

      <div>
        <label className={LABEL_CLASS}>Notes</label>
        <textarea id="form-notes" {...register('notes')} placeholder="Interview rounds, contacts, important details..." rows={3} className={`${FIELD_CLASS} resize-none`} />
      </div>
