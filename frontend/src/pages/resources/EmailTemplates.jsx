      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10 space-y-8">
        ...
        <div className="space-y-8">
          {templates.map(t => (
            ...
            >
              {/* Template Header */}
              <div className="px-6 py-4 border-b border-white/5 bg-white/2 flex items-center justify-between gap-4">
                <h3 className="font-bold text-sm text-white/90">{t.role}</h3>
                <button
                  onClick={() => handleCopy(t.id, `Subject: ${t.subject}\n\n${t.body}`)}
                  className="px-3 py-1.5 rounded-lg bg-indigo-600/20 border border-indigo-500/30 text-[10px] font-semibold text-indigo-300 hover:bg-indigo-600 hover:text-white transition-all flex items-center gap-1.5 cursor-pointer"
                >
                  {copiedId === t.id ? (
                    <>
                      <Check size={10} />
        ...
        {/* CTA section */}
        <div className="glass rounded-3xl p-8 border border-white/5 bg-gradient-to-r from-indigo-950/10 to-[#080820] text-center space-y-4 max-w-2xl mx-auto">
