  ...
  return (
    <header className="h-16 px-4 md:px-8 flex items-center justify-between bg-[#080820]/80 backdrop-blur-xl border-b border-white/8 sticky top-0 z-30 shrink-0">
      ...
      <div className="flex items-center gap-3">
          ...
          <AnimatePresence>
            {notifOpen && (
              <motion.div
                ...
                className="absolute right-0 top-12 w-80 glass-strong rounded-2xl shadow-2xl border border-white/10 overflow-hidden z-50"
              >
                {/* Header */}
        ...
        {/* ── AI Assistant button ────────────────────────────── */}
        <button
          onClick={() => { navigate('/copilot'); setNotifOpen(false) }}
          className="hidden md:flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 shadow-lg shadow-indigo-900/30 active:scale-95"
        >
          <Sparkles size={14} />
          AI Assistant
        </button>
