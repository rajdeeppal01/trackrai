...
import { Target, FileText, CheckCircle, AlertTriangle, Sparkles, Upload } from 'lucide-react';
...
export default function ATSMatcher() {
 ...
 <label htmlFor="jd-upload" className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-2xl bg-indigo-500/10 text-indigo-300 cursor-pointer hover:bg-indigo-500/20 border border-indigo-500/20 transition-colors ${parsingJD ? 'opacity-50 cursor-wait' : ''}`}>
 <Upload size={12} />
 ...
 className="flex-1 w-full bg-black/20 rounded-3xl p-4 text-sm text-white/80 placeholder-white/20 focus:outline-none focus:border-indigo-500/50 resize-none transition-colors"
 />
 </div>
 ...
 {resumeText ? (
 <div className="flex-1 w-full bg-emerald-500/5 border border-emerald-500/20 rounded-3xl p-6 flex flex-col items-center justify-center text-center">
 ...
 <div 
 className={`flex-1 w-full bg-black/20 border-2 border-dashed hover:border-purple-500/50 rounded-3xl p-6 flex flex-col items-center justify-center text-center transition-colors cursor-pointer ${parsingResume ? 'opacity-50 cursor-wait' : ''}`}
 ...
 className="w-full bg-black/60 rounded-2xl text-xs text-white px-2 py-2 outline-none focus:border-purple-500/50"
 >
 ...
 onClick={handleMatch}
 disabled={loading}
 className="w-full py-4 rounded-3xl bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-400 hover:to-purple-500 active:scale-[0.98] transition-all font-bold text-white flex items-center justify-center gap-2 shadow-lg shadow-purple-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {loading ? (
 <div className="flex items-center gap-2">
 ...
 <span key={i} className="px-3 py-1 bg-orange-500/10 border border-orange-500/20 text-orange-200 text-xs font-semibold rounded-2xl">
 {kw}
 </span>
 ))}
 </div>
 ) : (
 <p className="text-sm text-emerald-400/80 bg-emerald-500/10 p-3 rounded-3xl border border-emerald-500/20">
 ...
 <li key={i} className="flex gap-3 bg-white/5 p-3 rounded-3xl ">
