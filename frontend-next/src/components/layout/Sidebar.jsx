...
export default function Sidebar({ mobileOpen, onClose }) {
 ...
 <button
 onClick={onClose}
 className="w-7 h-7 rounded-2xl bg-white/5 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 transition-all md:hidden"
 >
 <X size={14} />
 </button>
 )}
 ...
 className="flex items-center gap-3 px-4 py-3 rounded-3xl text-sm font-medium transition-all duration-200 text-white/40 hover:text-white hover:bg-white/6"
 >
 {content}
 </a>
 );
  ...
  href={path}
  onClick={onClose}
  className={`flex items-center gap-3 px-4 py-3 rounded-3xl text-sm font-medium transition-all duration-200 ${
  isActive
 ...
 <div className="w-9 h-9 rounded-3xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-sm shrink-0">
 {avatarInitials}
 </div>
 ...
 onClick={logout}
 title="Log Out"
 className="w-8 h-8 rounded-2xl bg-white/5 hover:bg-red-500/10 text-white/40 hover:text-red-400 flex items-center justify-center transition-all cursor-pointer shrink-0"
 >
 <LogOut size={14} />
 </button>
 </div>
